...
- The currently attached accepted-T5/DriveMA snow run completed 94.3% of its inferred corridor with zero collisions and one lane invasion. Its evidence records 90% snow accumulation, a 9.81 m/s² CARLA gravity reference, 9.76 m/s² median measured IMU magnitude, and passing ground contact. The desktop defaults to the uncomposited native CARLA camera; the T5 view is explicitly labeled as a depth-aware visual composite, not unified CARLA geometry. T5 remains review-required, nonmetric, and `collisionValidated=false`.
